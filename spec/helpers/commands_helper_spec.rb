...
describe CommandsHelper do
  ...
  describe '#formatted_slack_response' do
    let(:expected_hash) {
      {
        response_type: 'in_channel',
        text: 'text1',
        attachments: [{ text: 'text2' }],
      }
    }

    it 'returns hash formatted for slack' do
      expect(formatted_slack_response('text1', 'text2')).to eql(expected_hash)
    end
  end
